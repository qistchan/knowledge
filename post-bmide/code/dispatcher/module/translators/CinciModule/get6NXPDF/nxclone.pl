...
my $zPath="${z_path}";
my $winzipPath="${winzip_path}";
my $zipoutPath="${zip_output_path}";
my $zipfileExt="${zip_file_name}";
if ( $^O =~ /MSWin/ )
  {
    $pathSep="\\";
    $shExt=".bat";
    ...
    $zPath="%z_path%";
	$winzipPath="%winzip_path%";
	$zipoutPath="%zip_output_path%";
	$zipfileExt="%zip_file_name%";
  }
# ZIP Switch (allowed values : WINZIP/7ZIP)
my $zipswitch="7ZIP";
...
$cloneArgs="$cloneArgs -default_d=$outputDir";
$cloneArgs="$cloneArgs -save_log_file=$saveLog";
...
print CLONE_FILE "$ENV{CLONE_COMMAND} $cloneLogin $cloneArgs  > $cloneOut 2>&1\n";
close CLONE_FILE;
...
print "winzipFile=$winzipFile\n";
print "zipswitch=$zipswitch\n";
if($zipswitch eq "WINZIP")
{
	open (ZIP_FILE, "> $winzipFile") or die "*** Error can't open $winzipFile ! \n";
	print ZIP_FILE "$shHeader\n";
	if ( $^O =~ /MSWin/ )
	{
		print ZIP_FILE "cd \/D $outputDir \n";
		print ZIP_FILE "FOR \/f \"tokens=*\" %%G IN ('dir $zipfileExt /b') DO (";
		print ZIP_FILE "$winzipPath\WINZIP64.EXE -min -e -o %%G $zipoutPath)";
	}
	else
	{
		print ZIP_FILE "cd \/D $outputDir \n";
...
if($zipswitch eq "7ZIP")
{
	...
	if ( $^O =~ /MSWin/ )
	{
		print ZIP_FILE "cd \/D $outputDir \n";
		print ZIP_FILE "FOR \/f \"tokens=*\" %%G IN ('dir $zipfileExt /b') DO (";
		print ZIP_FILE "$zPath\\7z.exe x %%G -aoa -o$zipoutPath)";
	}
	else
	{
		print ZIP_FILE "cd \/D $outputDir \n";
...
# Run zip command
#
print "executing unzip process...\n";
chmod (0755, $winzipFile);
system($winzipFile);
