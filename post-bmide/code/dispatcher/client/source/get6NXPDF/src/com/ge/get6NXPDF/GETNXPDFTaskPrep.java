...
extends com.teamcenter.ets.extract.TaskPrep
{
    ...
    */
    ...
    private static final String translatorlogFileName = "GE_get6NXPDF.log";
        ...
        try
        {
            ...
            for (int i = 0; i < zPrimaryObjs.length; i++)
            {
                ...
                if (secondaryObject != null) {
                    ...
                    } else if ((secondaryObject instanceof RevisionRule)) {
                        revRule = (RevisionRule)secondaryObject;
                    }
                }
                if (itemRev == null)
                {
                ...
                else
                {
                    ...
                    if (dataset != null)
                    {
                        ...
                        partString = getCLIName(scMfkId, revId, fMasterPart, datasetName, relationType);
                        anarkString = getAnarkCLIName(scMfkId, revId, fMasterPart, datasetName, relationType, this.stagingLoc);                    
                    }
                    else
                    {
                        partString = getCLIName(scMfkId, revId, true, null, null);
                    }
                    if (revRule != null)
                    {
                        String revisionRuleName = revRule.get_object_name();
                        zDBStringPrintStream.println("-revision_rule=\"" + revisionRuleName + "\"");
                    }
                    if (singlePart) {
                        zDBStringPrintStream.println("-part=\"" + partString + "\"");
                    } else {
                        zDBStringPrintStream.println("-assembly=\"" + partString + "\"");
