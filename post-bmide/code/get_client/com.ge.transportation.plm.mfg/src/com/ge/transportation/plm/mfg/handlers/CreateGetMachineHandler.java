...
import com.ge.transportation.plm.mfg.ui.CreateGetMachineItem;
 ...
 */
public class CreateGetMachineHandler extends AbstractHandler {
	/**
	 * The constructor.
	 */
	public CreateGetMachineHandler() {
	}
		...
		try {
			...
			if ((null != selectedComponents) && (selectedComponents.length == 1)) {
				...
				if ((selectedComponents[0] instanceof TCComponentFolder) || (selectedComponents[0] instanceof TCComponentBOMLine)) {
					...
					CreateGetMachineItem CreateGetMachineItemObj = new CreateGetMachineItem(window, selectedComponents);
					CreateGetMachineItemObj.createMachineItem();
				} else {
