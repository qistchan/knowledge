...
public class GETDataManagement
{
	...
	private static Map<Connection, GETDataManagement> m_instance;
	...
	*/
	...
	{
    	GETDataManagement returnInstance = null;
    	if(m_instance == null)
    	{
        	m_instance = new HashMap<Connection, GETDataManagement>();
    	...
    	{
        	returnInstance = new GETDataManagement(connection);
	...
	/**
     * Method to unload objects from memory
     * 
     * @param modelObjects - Array of ModelObject to be unloaded
     */
    public void unloadObjects(ModelObject[] modelObjects) throws Exception
    {
        ServiceData serviceData = null;
        serviceData = dmService.unloadObjects(modelObjects);
        String errorMessage = GETCommon.getErrorFromServiceData(serviceData);
        if (errorMessage != null && !errorMessage.isEmpty())
        {
            throw new Exception(errorMessage);
    ...
    /**
     * Method to refresh objects from memory
     * 
     * @param modelObjects - Array of ModelObject to be refreshed
     */
    public void refreshObjects(ModelObject[] modelObjects) throws Exception
    {
        ServiceData serviceData = null;
        serviceData = dmService.refreshObjects(modelObjects);
        String errorMessage = GETCommon.getErrorFromServiceData(serviceData);
        if (errorMessage != null && !errorMessage.isEmpty())
        {
            throw new Exception(errorMessage);
	...
	public List<String> getUids(ModelObject[] objs)
	{
		List<String> uidList = new ArrayList<String>();
		for(ModelObject obj : objs)
		{
			uidList.add(obj.getUid());
		}
    ...
    public Map<ModelObject, Map<ModelObject, Dataset[]>> getLatestCreatedRevs(ModelObject[] itemArr, Map<String, String[]> relnMap)
    {
        ...
        try
        {
           ...
           for(int itemIndex = 0; itemIndex < itemArr.length; itemIndex++)
           {
               ...
               itemAttrMap.put(GETConstants.BusinessObjects.Properties.strItemID, ((Item)itemArr[itemIndex]).get_item_id());
               itemAttrMap.put(GETConstants.BusinessObjects.Properties.strObjectType, ((Item)itemArr[itemIndex]).get_object_type());
     ...
     * @param revProcessing - String - To get info of specific revision("Ids") or no. of revisions("Nrev")
            ...
            List<Integer> nRevList, List<String> revIDList, List<Map<String, String[]>> datasetRelationFilterMapList)
    {
        ...
        try
        {
            ...
            for(Map<String, String> itemAttrInfoMap : itemAttrInfoMapList)
            {
                ...
                RevInfo revinfo = null;
                if ( nRevList != null || revIDList != null )
                {
                    revinfo = new RevInfo();
                ...
                if ( datasetRelationFilterMapList != null )
                {
            ...
            for(GetItemAndRelatedObjectsItemOutput itemOut :  itemAndRelatedObjectsResponse.output)
            {
                ...
                if ( itemOut.itemRevOutput != null )
                {
                    for(RevisionOutput revOut :  itemOut.itemRevOutput)
                    {
                        Map<Dataset, Map<String, Map<ModelObject, String>>> dsOutMap = null;
                        Map<String, Map<ModelObject, String>> relnOutMap = null;
                        if ( revOut.datasetOutput != null )
                        {
                            for(DatasetOutput dsOut :  revOut.datasetOutput)
                            {
                                ...
                                if ( relnOutMap == null )
                                {
                                    relnOutMap = new HashMap<String, Map<ModelObject, String>>();
                                }
                                relnOutMap.put(dsOut.relationTypeName, nrOutmap);
                                if ( dsOutMap == null )
                                {
                                    dsOutMap = new HashMap<Dataset, Map<String, Map<ModelObject, String>>>();
                                }
    ...
    /**
     * Function to get items for given itemids - supports MFK (for item_id attribute) 
     * 
     * @param itemIdList - List of item ids
     * @return List of Items
     * @throws Exception
     */
    public List<Item> getItemsFromItemID ( List<String> itemIdList ) throws Exception
    {
        List<Item> resultItemList = null;
        GetItemFromAttributeInfo[] attrInfoArr = new GetItemFromAttributeInfo[itemIdList.size()];
        ...
        for (String itemId : itemIdList)
        {
            ...
            itemIdMap.put(GETConstants.BusinessObjects.Properties.strItemID, itemId);
            attrInfoArr[index] = new GetItemFromAttributeInfo();
            attrInfoArr[index++].itemAttributes = itemIdMap;
        }

        GetItemFromAttributeResponse resp = dmService.getItemFromAttribute(attrInfoArr, 0, pref);
        if ( resp.output != null )
        {
            for ( GetItemFromAttributeItemOutput out : resp.output )
            {
                if ( resultItemList == null )
                {
                    resultItemList = new ArrayList<Item>();
                }
                resultItemList.add(out.item);
            }
        }
	 ...
	 */
    public List<String> getPropertryValues(ModelObject mo, String attr, boolean propsLoaded)
    {
    ...
    public ServiceData setProperties(ModelObject[] mo, Map<String, VecStruct> propMap)
    {
    	return dmService.setProperties(mo, propMap);
    }
    
    public ServiceData deleteObjects(ModelObject[] objsToDelete) throws Exception
    {
    	return dmService.deleteObjects(objsToDelete);
    }
}
